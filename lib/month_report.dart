...
//
// П'ять окремих речей довелось виправляти по черзі, жодна не була тим,
// що здавалось спочатку:
...
//     причина. Перша спроба — розбити на "голову" (дата + перші ~80
//     символів, нероздільно) і spanning "хвіст" — прибрала саме цей
//     симптом, але для звичайних нетривіальних заміток (~100 символів)
//     хвіст виходив крихітним ("два слова на новій сторінці самі по
//     собі") — так само зламано виглядає, лише зсунуто на рівень
//     нижче.
// (5) Тому розбиття на голову+хвіст тепер УВІМКНЕНО ЛИШЕ для нотаток
//     довших за `_noteSplitThreshold` (400 симв.) — усе коротше
//     лишається ОДНИМ нероздільним блоком, який просто цілком
//     переїжджає на наступну сторінку, якщо не влазить (дрібна втрата
//     місця, але без жодного зламаного вигляду). Розбиття лишається
//     тільки для справді довгих нотаток, де залишок після голови й так
//     буде щонайменше на абзац.
...
// датою, КОЛИ запис узагалі розбивається (див. пункт (4) вище й
// _noteSplitThreshold нижче) — досить, щоб дата ніколи не виглядала
// самотньою, замало, щоб помітно "з'їдати" бюджет розбиття для справді
// довгих нотаток.
const _noteHeadMinChars = 80;

// Записи ДОВШІ за це взагалі не намагаються розбитись на голову+хвіст —
// цілком non-spanning, як єдиний блок. Виявлено на реальному контенті:
// перша версія розбивала будь-яку нотатку довшу за 80 символів, і для
// звичайного речення "трохи довшого за 80" (~100 символів) хвіст
// виходив крихітним ("в ходовой." саме по собі на новій сторінці) —
// технічно дата вже не сама, але виглядає так само зламано, просто
// зсунуто на один рівень. Розбиття варте цієї ціни лише для СПРАВДІ
// довгих нотаток, де залишок після голови однаково буде щонайменше на
// абзац, а не на два слова.
const _noteSplitThreshold = 400;
...
List<pw.Widget> _buildNoteEntryWidgets(CheckinEntry entry, double rightGap) {
  ...
  String? head = note;
  String? tail;
  if (note != null && note.length > _noteSplitThreshold) {
