...
class _TimeCapsulesScreenState extends State<TimeCapsulesScreen> {
  ...
  Future<void> _openLetter(_Letter letter) async {
    ...
    if (state == _LetterState.unlockedUnread && letter.body == null) {
      // На відміну від просто "ще замкнено" (де стан і так наперед
      // відомий) — тут клієнт уже ВВАЖАЄ, що розкрито, просто ще не має
      // свіжих даних із сервера. Перезавантажуємо список замість того,
      // щоб просто показати повідомлення й нічого не зробити — інакше,
      // якщо реальний час розблокування вже настав (а це найчастіший
      // випадок такого стану, не розсинхронізований годинник), юзер
      // назавжди застряг би на "ще замкнено" для листа, який насправді
      // вже можна прочитати, аж доки не піде з екрана і не повернеться.
      // Наступний тап уже підхопить свіже тіло листа, якщо воно є.
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
