        ...
        children: [
          hasPhoto
              ? Stack(
                  fit: StackFit.expand,
                  children: [
                    // Кадр картки завжди 9:16, а не kCompactPhotoAspectRatio
                    // (4:3, рамка позиціювання при чек-іні) — свідомо інша
                    // пропорція, під власні потреби картки для шеру, не
                    // канонічна для кадрування. photoAlignX/Y/Scale ті самі,
                    // що юзер підбирав у 4:3-рамці, тож тут кадр може
                    // виглядати інакше (BoxFit.cover обрізає по-різному
                    // залежно від пропорції) — прийнятний компроміс саме
                    // для цієї картки, не для основного перегляду. Живе
                    // прев'ю в редакторі позиціювання (`photo_reposition_
                    // screen.dart`) показує це наперед.
                    ScaledPhoto(
