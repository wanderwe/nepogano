...
/// Пропорція і рамки позиціювання (`PhotoRepositionScreen`), і компактних
/// прев'ю фото скрізь у застосунку (запис в Історії, у друга, підсумок на
/// головному екрані) — та сама константа навмисно: якщо вони розійдуться,
/// юзер побачить у пості не те, що обрізав рамкою (реальний фідбек: "бачу
/// 1/3 фото в редакторі, після публікації бачу 2/3" — `BoxFit.cover`
/// обрізає по-різному залежно від пропорції рамки, тож редактор і
/// перегляд мусять бути однаковою пропорцією, не просто "схожою"). 4:3, не
/// квадрат і не стара пласка смужка (~2:1-3.8:1 залежно від ширини екрана,
/// зрізала занадто багато кадру) — компроміс, який економить вертикальний
/// простір списку, лишаючись не надто пласким. Картка дня
/// (`day_card_screen.dart`) — окрема історія, 9:16, свідомо інша
/// пропорція, не ця константа.
